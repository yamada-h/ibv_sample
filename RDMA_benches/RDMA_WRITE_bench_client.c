...
int main(int argc, char **argv)
{
    ...
    //check variables sent from client
    printf("server's qp_num : %d, addr : %p, r_key : %u\n", server_info.qpnum, server_info.addr, server_info.rkey);
   
    //Init -> RTR
    struct ibv_qp_attr rtr_attr = {
	.qp_state               = IBV_QPS_RTR,
        .path_mtu               = IBV_MTU_4096,
        .dest_qp_num 		= 691, //change this value for your environment
        .rq_psn                 = 0,
   ...
   struct ibv_send_wr send_wr = {
       ...
       .send_flags = 0,
       .wr.rdma.remote_addr = (uint64_t *)server_info.addr,
       .wr.rdma.rkey = server_info.rkey
   };
